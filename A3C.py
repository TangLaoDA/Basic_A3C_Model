...
class ACNet(object):
    ...
    def _build_net(self, scope):
        ...
        with tf.variable_scope('actor'):
            ...
            l_ah = tf.layers.dense(l_a, 200, tf.nn.relu6, kernel_initializer=w_init, name='l_ah')
            a_prob = tf.layers.dense(l_ah, N_A, tf.nn.softmax, kernel_initializer=w_init, name='ap') # 得到每个动作的选择概率
        with tf.variable_scope('critic'):
            l_c = tf.layers.dense(self.s, 100, tf.nn.relu6, kernel_initializer=w_init, name='lc')
            l_ch = tf.layers.dense(l_c, 100, tf.nn.relu6, kernel_initializer=w_init, name='l_ch')
            v = tf.layers.dense(l_ch, 1, kernel_initializer=w_init, name='v')  # 得到每个状态的价值函数
